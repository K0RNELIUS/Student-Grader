...
def line_print(biggest_string_len, number_of_columns, line_header, grades):
    ...
    for i in range(number_of_columns - len(grades)):
        line += f' {"Not Used".center(biggest_string_len)} |'
    return line
...
def student_search(student_name):
    ...
    for subject in database:
        if student_name in database[subject][3].keys():
            student_subjects.append(subject)
            if len(subject) > biggest_string:
                biggest_string = len(subject)
            if len(database[subject][3][student_name]) > biggest_columns:
                biggest_columns = len(database[subject][3][student_name])
    header_grades = []
    ...
    if len(student_subjects) != 0:
        ...
        for student_subject in sorted(student_subjects):
            grades = database[student_subject][3][student_name]
            print(line_print(biggest_string, biggest_columns, student_subject, grades))
            print(separator)
...
def teacher_login():
    ...
    while login_cond:
        ...
        if not name_check(teacher_name): # checks if name is in database
            ...
            teacher_password = input("Please type your password: ")
    ...
    while cont_cond:
        ...
        # 2- Update student grade
            ...
            if student_name in database[teacher_subject][3]:
                ...
                if not update_which.isalpha():
                    if update_which.isdigit():
                        if int(update_which) in range(1, len(grades) + 1):
                            update_value = input("What is the updated value (between 0 and 10)? ")
                            if not update_value.isalpha():
        ...
        # 4 - List students grades
            ...
            biggest_string = len("Student Name")
            biggest_columns = database[teacher_subject][2]
            header_grades = []
            for i in range(1, biggest_columns + 1):
                header_grades.append(f'Grade {i}')
            ...
            for student in sorted(students):
                ...
                print(line_print(biggest_string, biggest_columns, student, grades))
                print(separator)
        # 5 - End teacher command session
        elif option == "5":
            cont_cond = False
        else:
